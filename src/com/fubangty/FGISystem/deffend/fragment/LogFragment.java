...
import com.fubangty.FGISystem.deffend.activity.FillReportActivity;
import com.fubangty.FGISystem.deffend.activity.UnReportActivity;
...
public class LogFragment extends Fragment implements MyCallback {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		context = getActivity();
		View v = inflater.inflate(R.layout.activity_dailylog, null);
		tvReport = (TextView) v.findViewById(R.id.tv_log_report);
		lvDailyLog = (ListView) v.findViewById(R.id.lv_log_info);
		rbLogUnreport = (RadioButton) v.findViewById(R.id.rb_log_unreport);
		dbHelper = new DailyLogDBHelper(context, "mydailylog.db", null, 1);
		setListener();
		return v;
	}
	...
	public void sendMessage(int type, String pageCount, String currPage) {
		...
		CallHttpBiz call = new CallHttpBiz(context);
...
//		params.addBodyParameter("phoneID", phoneID); // 手机IMEI
		params.addBodyParameter("phoneNum", "15702330939"); // 手机号码(登录名)
		params.addBodyParameter("currMonth", getSystemTime()); // 当前月份
		params.addBodyParameter("pageCount", pageCount); // 每页显示条数(默认5)
		params.addBodyParameter("currPage", currPage); // 当前页(默认为1)
		//接口
		String url = "http://192.168.3.123:8080/data_service/wish/findAppLog.do";
//		call.PostData(2, new ConnectionUrl().LOGHASREPORT, params, this);
		call.PostData(2, url, params, this);
	}
	...
	public void onFailure(HttpException exception) {
		...
		Toast.makeText(context, "获取已上报数据失败！请检查网络配置或服务器", Toast.LENGTH_LONG).show();
	}
