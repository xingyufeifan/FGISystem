...
public class VideosActivity extends Activity implements AnyChatBaseEvent {
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			try {
				...
				{
					if (videoBitrate <= 0){						
						Toast.makeText(VideosActivity.this, "对方视频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
				...
				if (mFirstGetAudioBitrate){
					if (audioBitrate <= 0){
						Toast.makeText(VideosActivity.this, "对方音频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
...
//		onDestroy();
	...
	protected void onDestroy() {
		...
		anychatSDK.Release();
		//发送广播
		System.out.println("退出通话界面，执行ondestroy（）");
		Intent intent = new Intent(Global.stop_service);
...
//		finish();
	...
	public void OnAnyChatUserAtRoomMessage(int dwUserId, boolean bEnter) {
		if (!bEnter) {
			if (dwUserId == userID) {
				Toast.makeText(VideosActivity.this, "对方已离开！", Toast.LENGTH_SHORT).show();
				userID = 0;
